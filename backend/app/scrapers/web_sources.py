...
class IndeedScraper(WebScraperSource):
    ...
    def search(self, job_title: str, location: str = "France", limit: int = 50) -> List[dict]:
        ...
        try:
            ...
            for url in urls:
                ...
                if not html:
                    logger.warning(f"[WEB:Indeed] empty page for {url[:120]}")
                    continue
                ...
                if not cards:
                    logger.warning(f"[WEB:Indeed] no cards matched for {url[:120]}")
                    continue
        ...
        logger.info(f"[WEB:Indeed] done jobs={len(jobs)}")
        return jobs[:limit]
