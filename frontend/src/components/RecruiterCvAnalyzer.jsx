...
function RecruiterCvAnalyzer({ lang, onAnalysisSuccess, cvData: externalCvData }) {
    ...
    useEffect(() => {
        try {
            ...
            console.error('[RecruiterCvAnalyzer] Error syncing external cvData:', err);
        }
    }, [externalCvData]);
    ...
    const processFile = async (file) => {
        ...
        try {
            const response = await fetch(`${API_BASE}/api/recruiter/analyze-cv`, {
                method: 'POST',
                body: formData,
            });
            ...
            if (!response.ok) {
                ...
                if (contentType && contentType.includes('application/json')) {
                    try {
                        ...
                        console.error('[RecruiterCvAnalyzer] Error parsing error response:', e);
                    }
            ...
            console.error('[RecruiterCvAnalyzer] Analysis error:', err);
    ...
    return (
        <div className="card" style={{ padding: '20px' }}>
            ...
            <div
                ...
                style={{
                    border: `2px dashed ${dragActive ? 'var(--color-orange-500)' : 'var(--border-color)'}`,
                    borderRadius: 'var(--radius-md)',
                    padding: '32px 16px',
                    textAlign: 'center',
                    cursor: 'pointer',
                    background: dragActive ? 'var(--color-orange-50)' : 'var(--color-surface)',
                    transition: 'all 0.2s',
                }}
            >
                <input
                ...
                {loading ? (
                    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '8px' }}>
                        <Loader2 size={32} style={{ animation: 'spin 1s linear infinite', color: 'var(--color-orange-500)' }} />
...
export default RecruiterCvAnalyzer;